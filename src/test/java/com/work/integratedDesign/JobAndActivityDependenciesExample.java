...
/**
 * 车辆路径问题中定义和实施 任务之间的强依赖关系
 * 三个名为 "get key"、"use key" 和 "deliver key" 的任务必须出现在同一条路径中，并且它们必须连续出现，中间不能夹杂其他任务。
 */
public class JobAndActivityDependenciesExample {
    ...
    //检查 "get key"、"use key"、"deliver key" 是否都在同一个路径中，如果其中一个任务已经被分配到某条路径，那么后续相关任务也必须分配到这条路径。
